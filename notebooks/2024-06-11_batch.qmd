...
Next, edit your Nextflow configuration file as follows to specify the AWS configuration (replacing QUEUE_NAME with the name of your AWS Batch job queue)[^profile]:

[^profile]: On the latest version of the v2 pipeline, most of these options are already configured appropriately when running the pipeline with `-profile batch` (which is also the default profile). In this case, you only need to change `process.queue` to point to the name of your job queue.
